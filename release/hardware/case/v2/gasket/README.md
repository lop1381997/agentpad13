...
>
> **v2.6 update (2026-07-23).** The band's SIDEWALL was thickened (`WALL`
> 2.4 → 3.0, owner order after PCBWay's thin-crescent EQ), so the band STL hash
> changed and the **`36980cc2ff011dc32d923fb04f7429f7` invariance gate is
> RETIRED by owner order** (that hash = the superseded 2.4-wall band; do not
> print it). **This kit is unchanged-valid:** the ledge is INNER-derived and the
> band grew outward only — the rebuild removed zero material and added zero
> material into any mating void, and `gen_gasket.py` re-measures the ledge from
> the new band to the same numbers. `gasket_template.svg` and `.png` regenerate
> byte-identical.
>
> **v2.7 update (2026-07-24) — DEFAULT WALL now 5.4.** Owner decision; the
> shipping band is `agentpad13_v2_band_1.6mm_w5.4.stl`
> (`34be6bf79a6bb81995807448639f4822`), with `_w3.0`
> (`887b2538619db46d63b07cf044762bab`) and `_w7.4`
> (`163669962a928793a4e65347a80e4cfe`) as gated variants. **This kit is
> STILL unchanged-valid, and now for all three walls:** re-run against the 5.4
> band it re-measures the west ledge underside flat run at `x [-0.300, 0.900]
> @ z 0.3` and the ring at `84.8 × 100.6 / 82.4 × 98.2, width 1.2` — the values
> in the table below, unchanged — and `gasket_template.svg` / `.png` regenerate
> **byte-identical** (`527bba7d1ed7b545caed6f37d2197c02` /
> `4568f75a886266aef3597a21d1fd54aa`). `.pdf` / `.dxf` differ only in embedded
> timestamps/GUIDs. The kit is INNER-derived, so it is the same part at every
> wall.
...
ring is cut in `band()` of `../agentpad13_case_v2.py` from these constants
(read there, never retyped):

| quantity | value | source |
|---|---|---|
| ledge width `LEDGE_W` | `1.2 mm` | `agentpad13_case_v2.py` |
| ledge underside `LEDGE_Z0` | `+0.3 mm` above PCB top | `agentpad13_case_v2.py` |
| band inner wall `INNER_W × INNER_H` (ledge OUTER outline) | `84.8 × 100.6 mm`, R`5.6` | derived `PCB + 2·PCB_CLEARANCE`; R is the FROZEN `INNER_R` (v2.6 — was `OUTER_R − WALL`, pinned so a wall change cannot move the ledge, the plate's R5.4 or the tray's R5.35) |
| ledge INNER outline `INNER_ − 2·LEDGE_W` | `82.4 × 98.2 mm`, R`4.4` | derived |
| PCB outline | `84.2 × 100.0 mm` octagon | `contract_v4.json` |
...
| `gen_gasket.py` | regenerates all of the above from the case-model constants |
...
Regenerate: `python3 gen_gasket.py` (needs `inkscape` + `ezdxf` + `pymupdf`;
asserts the band md5 is unchanged before emitting anything).
