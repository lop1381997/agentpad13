...
> CPL, `gerbers_v5_6.zip`, `build_manifest.json`, `build_report.txt` and
> `source.kicad_pcb` is byte-identical.** `verify_fabpack.py` grew a new
...
> ties `TOUCH_AHLB → GND` on `v5_6.kicad_pcb`, and on a TTP223 an AHLB low
...
> **`v5_6` remains the FABRICATED and populated revision — the hardware on the
> owner's desk is v5_6 and is not recalled by this.** `v5_7` is the public-release
> and future-spin fix; it is **not** a rescue of hardware in hand, and the boards in
> hand keep their outward-firing bottom pair. Lineage hash `221ebb98…` is retained
> in MANIFEST's canonical-lineage table for exactly that reason. Full detail in diff
> row **M**; firmware, plate and case artifacts are **untouched**.
